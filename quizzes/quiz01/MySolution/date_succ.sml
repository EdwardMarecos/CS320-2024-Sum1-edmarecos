...
fun date_succ( date : date) : date =
    ...
    in 
        case date (year mod 4 = 0, month mod 2 = 0, month = 2, month = 12, day = 30, day = 31, day = 28, day = 29)
            (true, _, true, _, _, _, _, true)   => makeDate(year, 3, 1)             (* feb 29 to march 1*)
          | (false, _, true, _, _, _, true, _)  => makeDate(year, 3, 1)             (* feb 28 to march 1 on non leap year*)
          | (_, _, _, true, _, true, _, _)      => makeDate(year + 1, 1, 1)         (* dec 31 to jan 1 new year*)
          | (_, _, _, _, _, true, _, _)         => makeDate(year, month + 1, 1)     (* months with 31 days to next month*)
          | (_, false, _, _, true, _, _, _)     => makeDate(year, month + 1, 1)     (* other months *)
